...
const ApartmentCard = ({ apartment }) => {
    ...
    return (
        ...
        <div className="col-md-6 col-lg-4 my-3">
            <div class="card">
                <img class="card-img-top apartmentCard-img" src={imgMain} alt={appName} />
                <div class="card-body">
                    <h5 class="card-title">{appName}</h5>
                    <div className="details">
                        <div className="d-flex align-items-center mb-2">
                            <span className="pr-2">{locationIcon}</span><span><p class="card-text">{address}</p></span>
                        </div>
                        <div className="bed-bath d-flex">
                            <div className="d-flex align-items-center">
                                <span className="pr-2">{bedIcon}</span><span><p class="card-text">{bed} Bedrooms</p></span>
                            </div>
                            <div className="d-flex align-items-center">
                                <span className="pr-2">{bathIcon}</span><span><p class="card-text">{bath} Bathrooms</p></span>
                            </div>
                        </div>
                    </div>
                    <div className="footer d-flex align-items-center pt-4">
                        <div className="price brand-color">${price}</div>
                        <div className="details-btn"> <Link to={"apartment/" + _id} className="btn brand-btn">View Details</Link> </div>
                    </div>
                </div>
